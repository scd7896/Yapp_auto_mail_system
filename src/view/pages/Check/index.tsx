...
const App = () => {
  ...
  const reSendMails = () => {
    const targetResendUsers: Array<User> = userList.filter((el: User) => {
      return el.isError === true;
    });
    setUserList([]);
  ...
  useEffect(() => {
    if (socket !== null) {
      ...
      axios.default.get(`${url}/sendmail`).then((value) => {
  ...
  return (
    <div>
      <div>
        <div className="">
          <button onClick={startSendMails} disabled={socket !== null}>
            소켓 열고 시작하기
                    </button>
          <button onClick={reSendMails} disabled={allUserCount === userList.length}>
            재전송
                    </button>
        </div>
      </div>
